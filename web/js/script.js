$(function() {
	...
	function scrollRelation() {
		var fromTop = $(this).scrollTop(),
			...
			curr = scrollItems.map(function() {
				if ($(this).offset().top -($(this).attr('id') == 'contacts'?200:0) <= fromTop + headerHeight + ((windowHeight >= 480 && windowWidth >= 480 && windowWidth < 1440 && $(this).attr('id') != 'about' && $(this).attr('id') != 'knowledge') ? 50 : 0))
					return this;
			});
	...
	$('header.header .nav a, footer.footer a, .nav-ctrl a, header.header .top').click(function(e) {
		...
		var offsetTop = (href === '#') ? 0 : $(href).offset().top - headerHeight - ((windowHeight >= 480 && windowWidth >= 480 && windowWidth < 1440 && href != '#about' && href != '#knowledge') ? 49 : 0);

		if (href == '#about' || href == '#steps' || href == '#') {
	...
	$('#contacts .success a.close').click(function(e) {
		$('#contacts div.success').removeClass('open');
		e.preventDefault();
	});
	...
	function map_init() {
		...
		var marker_pic = {url: 'src/map.png', size: new google.maps.Size(55, 74), origin: new google.maps.Point(0, 0), anchor: new google.maps.Point(22, 37), scaledSize: new google.maps.Size(55, 74)};
		var markerOptions = {
	...
	var stepSlider = function () {
		...
		that.onWheel = function (event) {
			...
			if (Math.abs(screenPos - sliderPos) <= that.delta && (scrollPos + 108) >= offsetPos) {
				if (Math.abs(that.lastChangeTime - Date.now()) < 1000) {
					return false;
				} else {
		...
		that.touchInit = function () {
			...
			$('#steps').on( "swipeleft", that.next);
			$('#steps').on( "swiperight", that.prev);
		};
